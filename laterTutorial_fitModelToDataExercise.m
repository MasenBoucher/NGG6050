...
% EXERCISE:
initialValues = [mean(1./RTs),std(1./RTs)];
...
% Note -- needed to install packages to get 'optimoptions' to work
...
% Note -- Had an issue with this code and had to remove arguments from "fits" to get it to run
[fits, nllk] = run(gs,problem); % Had to be edited to get code to run 
...
%Creating figure
figure;
histogram(1./RTs,'Normalization','pdf')
hold on;
...
%Calculate Gaussian function for x
y = normpdf(x,mu,sigma);

%Plotting figure
plot(x,y,'LineWidth',1.5)
xlabel('Time (s)')
ylabel('Percent of Counts (as decimal)')